                                ...
                                if ((fillChar >= 32 && fillChar <= 126) || (fillChar >= 160 && fillChar <= 255)) {
                                    ...
                                    for (int row = top - 1; row < bottom; row++)
                                        for (int col = left - 1; col < right; col++)
                                            if (!selective ||
                                                (TextStyle.decodeEffect(mScreen.getStyleAt(row,
                                                        col)) &
                                                 TextStyle.CHARACTER_ATTRIBUTE_PROTECTED) == 0) {
                                                mScreen.setChar(col, row, fillChar,
                                                        keepVisualAttributes ? mScreen.getStyleAt(
                                                                row, col) : style);
                                            }
                                }
                                break;
            ...
            if (code == 0) { // reset
                ...
                if (firstArg == 2) {
                    ...
                    if (color >= 0 && color < TextStyle.NUM_INDEXED_COLORS) {
                        ...
                        if (LOG_ESCAPE_SEQUENCES) {
                            mClient.logWarn(LOG_TAG, "Invalid color index: " + color);
                        }
                    }
                } else {
    ...
    private void emitCodePoint(int codePoint) {
        ...
        if (mInsertMode && displayWidth > 0) {
            ...
            if (destCol < mRightMargin) {
                mScreen.blockCopy(mCursorCol, mCursorRow, mRightMargin - destCol, 1, destCol,
                        mCursorRow);
            }
        }
